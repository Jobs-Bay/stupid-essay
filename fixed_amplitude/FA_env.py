...
class STAR_RIS_env(object):
    ...
    def normalize_Phi(self, Phi_t_real, Phi_t_imag, Phi_r_real, Phi_r_imag):
        ...
        for i in range(self.element_num):
            ...
            Phi_t[i] = math.sqrt(0.5) * math.cos(theta_t[i]) + 1j * math.sin(theta_t[i]) * math.sqrt(beta_t[i])
            Phi_r[i] = math.sqrt(0.5) * math.cos(theta_r[i]) + 1j * math.sin(theta_r[i]) * math.sqrt(beta_r[i])
        return Phi_t, Phi_r
