...
proposed_env = proposed_env(antenna_num=4, user_num=4, element_num=30, power_limit=-10, target_num=4, eve_num=1)
ddpg = DDPG(a_dim=proposed_env.action_dim, s_dim=proposed_env.state_dim, a_bound=[-1, 1])
saver = tf.train.Saver()
with ddpg.sess as sess:
    # 恢复模型参数
    saver.restore(sess, "ddpg/数据/decay_random/LA=1e-05,LC=1e-05,GAMMA=0.5/M=4,K=4,N=30,P=0,T=4,F=1")  # 请根据你的模型路径修改
